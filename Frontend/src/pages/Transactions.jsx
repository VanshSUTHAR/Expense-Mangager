...
import { FaCreditCard, FaEdit, FaTrash, FaWallet, FaFilePdf, FaFileExcel } from "react-icons/fa";
import { FaArrowTrendUp, FaArrowTrendDown } from "react-icons/fa6";
import { FaSave, FaPlus } from "react-icons/fa";
import "./Transactions.css";
import { TbTransfer } from "react-icons/tb";
import { exportTransactionsPDF, exportTransactionsExcel } from "../utils/exportData";
...
export default function Transactions() {
  ...
  const switchPaymentType = (type) => {
    ...
    setForm(prev => ({
      ...
      bankName: "",
      accountNumber: "",
      paymentMethod: type === "credit" ? "credit_card" : type === "debit" ? "debit_card" : "bank",
    }));
  };
  ...
  const applyFilters = () => {
    ...
    if (filters.type === "loan_emi") {
      filtered = filtered.filter(tx => tx.isLoanEMI || tx.category === "loan_emi");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.type === "expense" && selectedDebitCardId && debitCardBalance !== null) {
        const txAmount = Number(form.amount);
        if (txAmount > debitCardBalance.balance) {
          toast.error(`Insufficient Balance. Your account contains only ₹${Number(debitCardBalance.balance).toLocaleString("en-IN")}.`);
          return;
        }
      }

      // Frontend balance check for direct bank account expense
      if (form.type === "expense" && directBankAccountId && !selectedDebitCardId) {
        const acc = bankAccounts.find(b => b._id === directBankAccountId);
        if (acc && Number(form.amount) > acc.balance) {
          toast.error(`Insufficient Balance. ${acc.bankName} contains only ₹${Number(acc.balance).toLocaleString("en-IN")}.`);
          return;
        }
      }
      ...
      const payload = {
        ...
        debitCardId: form.type === "expense" && selectedDebitCardId ? selectedDebitCardId : undefined,
        bankAccountId: form.type === "income"
          ? (incomeBankAccountId || undefined)
          : form.type === "expense" && directBankAccountId && !selectedDebitCardId
            ? directBankAccountId
            : undefined,
      };
  ...
  // Credit card bills can only be paid from a bank account or debit card — never another credit card
  const buildPaymentSourceOptionsHtml = () => {
    ...
    const debitOptions = debitCards.map((card) => {
  ...
  const handlePayBill = async (bill) => {
    try {
      const outstanding = Number(bill.outstandingAmount || bill.totalAmount || 0);
      ...
      const result = await Swal.fire({
        ...
        preConfirm: () => {
          ...
          if (amountValue > outstanding) {
            Swal.showValidationMessage(`Maximum payable amount is ₹${outstanding.toLocaleString('en-IN')}`);
            return false;
          }

          const src = JSON.parse(decodeURIComponent(sourceValue));
          if (amountValue > src.balance) {
            Swal.showValidationMessage(`Insufficient Balance. Account has only ₹${Number(src.balance).toLocaleString('en-IN')}`);
            return false;
          }
      ...
      await api.put(`/credit-card-bills/${bill._id}/pay`, { amount: paymentAmount });
      ...
      toast.success(`₹${paymentAmount.toLocaleString('en-IN')} paid successfully`);
      fetchCreditCardBills();
      fetchAllTransactions();
      fetchBankAccounts();
      fetchDebitCards();
  ...
  const handlePayFullBill = async (bill) => {
    try {
      ...
      await api.put(`/credit-card-bills/${bill._id}/pay`, { amount: outstanding });
      ...
      toast.success(`Full payment of ₹${outstanding.toLocaleString('en-IN')} completed`);
      fetchCreditCardBills();
      fetchAllTransactions();
      fetchBankAccounts();
      fetchDebitCards();
    } catch (err) {
      toast.error(err.response?.data?.message || "Error paying full credit card bill");
    }
  };
  ...
  const openAdd = useCallback(() => { setEditTx(null); resetForm(); setShowModal(true); }, []);
  ...
  const cardsWithLimit = savedCards.filter((card) => Number(card.creditLimit) > 0);

  const getCardBill = (card) => {
    const matchedBills = bills.filter((bill) => {
      const cardNameMatches = bill.cardName === card.cardName;
      const cardNumberMatches = card.cardNumber && bill.cardNumber === card.cardNumber;
  ...
  const getCardLimitInfo = (card) => {
    ...
    //   .reduce((sum, bill) => sum + Number(bill.outstandingAmount || bill.totalAmount || 0), 0);
  ...
  return (
    <div className="transactions-page fade-in">
      <div className="page-header">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button className="btn btn-secondary export-btn"
            onClick={() => { exportTransactionsPDF(displayTxs); toast.success("PDF exported!"); }}
            title="Export as PDF">
            <FaFilePdf size={14} color="#dc2626" /> PDF
          </button>
          <button className="btn btn-secondary export-btn"
            onClick={() => { exportTransactionsExcel(displayTxs); toast.success("Excel exported!"); }}
                ...
                <div key={i} className="credit-limit-card">
                  <div className="credit-limit-card-header">
                    ...
                    <div className="credit-limit-card-info">
                      <div className="credit-limit-card-name">{card.cardName}</div>
                      <div className="credit-limit-card-num">••••{card.cardNumber.slice(-4)}</div>
                    </div>
                    <div className={`credit-limit-status ${info.usedPercent >= 90 ? "danger" : info.usedPercent >= 70 ? "warning" : "safe"}`}>
                      {info.usedPercent >= 90 ? "⚠️ Critical" : info.usedPercent >= 70 ? "⚡ High" : "✅ Good"}
                    </div>
                  </div>
                  ...
                  <div className="credit-limit-bar-wrap">
                    <div className="credit-limit-bar">
                      <div
                        ...
                        style={{
                          ...
                          background: info.usedPercent >= 90
                    ...
                    <span className="credit-limit-pct">{info.usedPercent.toFixed(0)}%</span>
                  </div>
                  ...
                  <div className="credit-limit-amounts">
                    ...
                    <div className="credit-limit-amt">
                      <span>Used</span>
                      <strong style={{ color: "#dc2626" }}>₹{info.used.toLocaleString()}</strong>
                    </div>
                    <div className="credit-limit-amt">
                      <span>Available</span>
                      <strong style={{ color: "#059669" }}>₹{info.available.toLocaleString()}</strong>
                    </div>
                  </div>
      ...
      {showModal && createPortal(
        <div className="modal-overlay transaction-modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal glass fade-in transaction-edit-modal">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Income: deposit to bank account (MongoDB) */}
              {form.type === "income" && (
                <div className="form-group">
                  <label>Deposit to Account</label>
                  <select className="input" value={incomeBankAccountId}
                    onChange={e => {
                      const acc = bankAccounts.find(b => b._id === e.target.value);
                ...
                <div className="form-group">
                  ...
                  {paymentMethodType === "bank" && (
                    <div className="pay-method-panel">
                      <select className="input" value={directBankAccountId}
                        onChange={e => {
                          const acc = bankAccounts.find(b => b._id === e.target.value);
                          setDirectBankAccountId(e.target.value);
                          setForm({ ...form, bankName: acc?.bankName || "", accountNumber: acc?.accountNumber || "", paymentMethod: "bank" });
                        }}>
                        <option value="">— Select bank account —</option>
                        {bankAccounts.map(bank => (
                          <option key={bank._id} value={bank._id}>
                            {bank.bankName} ••••{bank.accountNumber.slice(-4)} — ₹{Number(bank.balance).toLocaleString("en-IN")}
                          </option>
                        ))}
                      </select>
                      {bankAccounts.length === 0 && (
                        <p className="pay-method-hint">No bank accounts yet. Add one in Settings.</p>
                      )}
                      {directBankAccountId && (() => {
                        const acc = bankAccounts.find(b => b._id === directBankAccountId);
                        if (!acc) return null;
                        ...
                        return (
                          <div className={`debit-balance-info ${insufficient ? "insufficient" : "sufficient"}`}>
                            {insufficient
                  ...
                  {paymentMethodType === "debit" && (
                    <div className="pay-method-panel">
                      <select className="input" value={selectedDebitCardId}
                        onChange={e => handleDebitCardSelect(e.target.value)}>
                        <option value="">— Select debit card —</option>
                        {debitCards.map(card => (
                          <option key={card._id} value={card._id}>
                            {card.cardName} ••••{card.cardNumber.slice(-4)} → {card.linkedBankAccount?.bankName}
                          </option>
                        ))}
                      </select>
                      {selectedDebitCardId && debitCardBalance !== null && (
                        <div className={`debit-balance-info ${Number(form.amount) > debitCardBalance.balance ? "insufficient" : "sufficient"}`}>
                  ...
                  {paymentMethodType === "credit" && (
                    <div className="pay-method-panel">
                      <select className="input" value={form.bankName}
                        onChange={e => {
                          const card = savedCards.find(c => c.cardName === e.target.value);
                          setForm({ ...form, bankName: card?.cardName || "", accountNumber: card?.cardNumber || "", paymentMethod: "credit_card" });
                        }}>
                        <option value="">— Select credit card —</option>
                        {savedCards.map((card, i) => (
                          <option key={i} value={card.cardName}>
                            {card.cardName} ••••{card.cardNumber.slice(-4)}{card.creditLimit > 0 ? ` — Limit ₹${Number(card.creditLimit).toLocaleString("en-IN")}` : ""}
                          </option>
                        ))}
                      </select>
                      <p className="pay-method-hint">Credit card expenses are tracked via monthly bill statements.</p>
                    </div>
                  )}
                </div>
              )}
